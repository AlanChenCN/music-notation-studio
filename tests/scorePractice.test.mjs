...
test('timeline accepts recovery and extra pitches during the recognition period', () => {
...
test('timeline settles incomplete chords, wrong-only input, and silence once per event', () => {
  ...
  assert.deepEqual(c.getSnapshot().liveWarningPitches,[])
  now=5500; c.tick(); assert.equal(c.getSnapshot().eventResults['0'],'missed')
  c.release('c'); c.release('e'); c.press('wrong',65)
  assert.deepEqual(c.getSnapshot().liveWarningPitches,[65])
  now=6500; c.tick(); assert.equal(c.getSnapshot().eventResults['1'],'missed')
  c.release('wrong'); now=7500; c.tick()
...
test('timeline pause retains an in-window hit and settled history', () => {
  ...
  now=9000; c.start(); now=10000; c.tick()
  assert.equal(c.getSnapshot().beat,1.5); assert.equal(c.getSnapshot().eventResults['0'],'correct')
  c.pause(); c.setTempo(120); c.start(); now=10500; c.tick()
  assert.equal(c.getSnapshot().beat,2.5); assert.equal(c.getSnapshot().eventResults['1'],'missed')
...
test('timeline range, mid-score count-in and looping preserve the previous result', () => {
  ...
  now=4100; play(c,[67]); now=5500; c.tick(); assert.equal(c.getSnapshot().rounds,1); assert.equal(c.getSnapshot().beat,0); assert.match(c.getSnapshot().result,/正确 1 项/)
...
test('timeline clips range boundaries and settles a cross-measure event only once', () => {
  ...
  now=3100; play(c,[60]); now=4500; c.tick()
  assert.equal(c.getSnapshot().eventResults['0'],'correct')
  play(c,[62]); now=6500; c.tick()
  assert.deepEqual(c.getSnapshot().eventResults,{0:'correct',1:'correct'})
  assert.equal(c.getSnapshot().completed,2)
})
test('timeline recognition band accepts early and late input without allowing out-of-order completion', () => {
  let now=0; const c=new ScorePractice(()=>now); c.load({...score([[60],[62],[64]]),tempo:60}); c.setMode('timeline'); c.start()
  now=4600; c.press('early-next',62)
  assert.equal(c.getSnapshot().recognitionEventId,'0'); assert.equal(c.getSnapshot().eventResults['1'],undefined)
  c.press('current',60)
  assert.equal(c.getSnapshot().eventResults['0'],'hit'); assert.equal(c.getSnapshot().eventResults['1'],undefined)
  c.release('early-next'); c.press('retry-next',62)
  assert.equal(c.getSnapshot().eventResults['1'],'hit')
  c.release('current'); c.release('retry-next')
  now=6400; c.press('late-third',64)
  assert.equal(c.getSnapshot().eventResults['2'],'hit')
  now=7500; c.tick()
  assert.deepEqual(c.getSnapshot().eventResults,{0:'correct',1:'correct',2:'correct'})

  now=0; const late=new ScorePractice(()=>now); late.load({...score([[60],[]]),tempo:60}); late.setMode('timeline'); late.start()
  now=5400; play(late,[60])
  assert.equal(late.getSnapshot().eventResults['0'],'hit')
  now=5500; late.tick(); assert.equal(late.getSnapshot().eventResults['0'],'correct')
})

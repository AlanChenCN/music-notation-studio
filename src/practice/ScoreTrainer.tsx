...
interface Props { label?: string; renderDocumentBar?: (busy: boolean) => ReactNode; beforeControls?: ReactNode; emptyContent?: ReactNode; noteNameMode?: PracticeNoteNameMode; controller: ScorePractice; active: boolean; inputHeld: boolean; onPlayNote: (pitch: number) => void; onStopNote: (pitch: number) => void }
const noEnd = () => {}
const emptyPitches: number[] = []
export default function ScoreTrainer({ label = '乐谱跟练', renderDocumentBar, beforeControls, emptyContent, noteNameMode = 'hidden', controller, active, inputHeld, onPlayNote, onStopNote }: Props) {
  ...
  return <section hidden={!active} className="score-editor score-trainer" aria-label={label}>
    {renderDocumentBar ? renderDocumentBar(inputHeld || state.running || playback.playing) : <div className="score-document-bar">
    ...
    {score.events.length ? <ScoreStaff noteNameMode={noteNameMode} readOnly score={score} selected={target?.id ?? null} beat={timeline && (state.running || !demoView) ? timelineBeat : rangeStart + playback.beat} playing={playback.playing || (timeline && state.running)} followLeft={timeline} livePitches={state.livePitches} liveWarningPitches={timeline ? state.liveWarningPitches : emptyPitches} liveAtCursor={timeline || playback.playing} practiceRecognitionBand={timeline && !demoView} practiceEventResults={timeline ? state.eventResults : undefined} previewPitches={emptyPitches} previewDuration={1} insertionIndex={target ? score.events.indexOf(target) : 0} onSelect={select} onEnd={noEnd} /> : <div className="trainer-empty">{emptyContent ?? <><strong>把自己的乐谱变成练习</strong><p>在 Editor 编写旋律或和弦后点击「去练习」，也可以载入此前保存的乐谱。</p><p>先听示范，再按自己的速度逐项练习。</p></>}</div>}

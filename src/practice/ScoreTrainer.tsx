...
export default function ScoreTrainer({ controller, active, inputHeld, onPlayNote, onStopNote }: Props) {
  ...
  const rangeEnd = Math.min(scoreLength(score), to * beats)
  const timelineTarget = timeline ? targets.find(event => Math.max(rangeStart, event.startBeat) <= state.beat && Math.min(rangeEnd, event.startBeat + event.duration) > state.beat) : undefined
  const target = timeline ? timelineTarget ?? (state.beat < state.startBeat ? targets[state.index] : undefined) : targets[state.index]
  ...
  return <section hidden={!active} className="score-editor score-trainer" aria-label="乐谱跟练">
    ...
    <p className="trainer-live-status">实奏：{state.livePitches.map(p => midiNumberToPianoNote(p)?.name).join(' · ') || '—'}{timeline && ` · 跟弹进度 ${Math.max(0, timelineBeat - rangeStart).toFixed(1)} / ${scoreLength(demo)} 拍`}</p>
    {state.result && <p className="trainer-result">上一轮：{state.result}</p>}
    <p className="score-hint">{timeline ? '按 BPM 自动推进，先预备一小节；整个时值格都是识别窗口，完整目标集合允许包含额外音。未命中的非目标实奏音仅用警告色提示，不扣分；窗口结束后每项只结算为正确或未通过。' : '和弦需同时按对所有音，全部松键后继续。强调色空心音符显示当前实奏，与蓝色目标比较音高。'} 点击范围内音符可重新选择起点；BPM 同时用于跟弹与示范播放。</p>
  </section>
}

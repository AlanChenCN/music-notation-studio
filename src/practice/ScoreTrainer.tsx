...
export default function ScoreTrainer({ controller, active, inputHeld, onPlayNote, onStopNote }: Props) {
  ...
  const target = timeline ? score.events.find(event => event.startBeat <= timelineBeat && event.startBeat + event.duration > timelineBeat) : targets[state.index]
  ...
  return <section hidden={!active} className="score-editor score-trainer" aria-label="乐谱跟练">
    ...
    <div className="trainer-metrics"><span>当前目标 <strong>{target?.pitches.map(p => midiNumberToPianoNote(p)?.name).join(' · ') || '—'}</strong></span><span>完成 <strong>{state.completed} / {targets.length - state.runStart}</strong></span><span>{timeline ? '命中率' : '首次正确'} <strong>{state.completed ? Math.round(state.firstTry / state.completed * 100) : 0}%</strong></span><span>错误 <strong>{state.errors}</strong></span>{timeline && <span>漏弹 <strong>{state.missed}</strong></span>}<span>用时 <strong>{state.seconds} 秒</strong></span><span>已完成轮次 <strong>{state.rounds}</strong></span></div>
    ...
    <p className="trainer-live-status">实奏（强调色）：{state.livePitches.map(p => midiNumberToPianoNote(p)?.name).join(' · ') || '—'}{timeline && ` · 跟弹进度 ${Math.max(0, timelineBeat - rangeStart).toFixed(1)} / ${scoreLength(demo)} 拍`}</p>
    {state.result && <p className="trainer-result">上一轮：{state.result}</p>}
    <p className="score-hint">{timeline ? '按 BPM 自动推进，先预备一小节；在目标时段内弹对完整音高即命中，错音或漏弹不会停轴，休止符按时经过。播放轴保持左侧，强调色空心音符显示实奏音高。' : '和弦需同时按对所有音，全部松键后继续。强调色空心音符显示当前实奏，与蓝色目标比较音高。'} 点击范围内音符可重新选择起点；BPM 同时用于跟弹与示范播放。</p>
  </section>
}

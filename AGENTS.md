...
- Trainer 新增乐谱跟练：接收 Editor 当前副本或载入已保存乐谱，支持逐音 / 时间轴跟弹、小节范围、循环、实奏谱面反馈与音高计分；随机练习生成标准乐谱并复用同一跟练界面；自由弹奏独立提供最近试音。详见 `TRAINER.md`。
...
- Editor 编辑光标和播放光标必须独立，均使用安全区跟随，不能强制居中或互相驱动。Trainer 时间轴跟弹采用独立的左侧 20% 固定播放轴，不能套用 Editor 的中央安全区。

...
- 修复时间轴判定：每个原始事件使用完整时值窗口；单音触发目标音即命中，和弦曾同时包含全部目标音即命中，并允许额外音及窗口内错后补对。
...
- Trainer 时间轴模式播放轴固定在可视区左侧 20%，谱面持续滚动，短谱及末尾也有足够留白；正式音符恢复在时值格内居中，识别窗口与音符中心解耦。窗口调整后恢复锚定，Editor 保持原有排版和安全区。
- 跟弹与示范共用 BPM，练习进度和示范定位独立。

验证：30 项自动化测试、构建与 Lint；覆盖额外音、错后补对、和弦完整性、连续同音新触键、暂停与循环状态、区间裁剪、跨小节事件、谱面居中与反馈渲染。

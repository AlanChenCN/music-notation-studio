...
## [v0.6.0] - 2026-09-06（开发中）

### 随机练习与自由弹奏升级（2026-09-07）

- Trainer 拆分三种独立来源；随机出题复用标准乐谱、逐音 / 时间轴跟弹、谱面反馈、范围循环和示范播放，保持与乐谱跟练相同布局。
- 新增四种预设、88 键音域、黑白键音符池、大 / 小三和弦、七种拍号、2–16 小节、五种可选时值、少量休止符与相邻音倾向。出题严格填满小节，无效配置不替换旧题。
- 支持出题设置折叠、音名提示、待生成参数和当前题目分离；生成谱可送到 Editor 编辑，确认替换、撤销恢复及手动保存边界明确。
- 自由弹奏统一文档栏与控制区，保留即时谱面 / 和弦识别，新增最近完整试音及清除；多输入来源同音松键不提前清除键盘高亮。
- 会话配置和生成谱暂不持久化，版本仍为 v0.6.0；旧随机模块暂保留，详见 TRAINER.md。

验证：39 项测试、构建与 Lint；浏览器验证随机生成、无效音域保护、逐音计分、来源切换、最近试音及清除。Editor 交接确认框触发浏览器自动化超时，交接 / 撤销待人工验收；MIDI 硬件未实测。

### 本地采样钢琴音源

- 将原有单一 `sine` 振荡器替换为多声部钢琴 Voice Engine：每个 `source:note` 独立持有声音，避免不同输入来源提前切断同音。
- 默认使用本地 Salamander Grand Piano V3 的 20 个 MP3 锚点样本，按 `playbackRate` 覆盖 `A0-C8`；加载期间保留轻量合成音 fallback，避免首次输入无声。
- 每个 Voice 使用短 Attack、自然 Release 和 MIDI Velocity；全部声音统一经过 Master Gain 与 Dynamics Compressor，改善和弦音量叠加、爆音和断层感。
- Browser Sound 开关保持原有设置语义，仅控制音频总线；采样署名与许可证见 [ATTRIBUTIONS.md](ATTRIBUTIONS.md)。

### Trainer 时间轴跟弹整改

- 优化乐谱播放刷新：移除 Editor 重复的 16ms 定时器，统一由单一 RAF 播放时钟驱动；Trainer 时间轴跟随浏览器绘制帧更新，播放滚动在绘制前同步，减少 SVG 播放抖动。
- 保留逐音练习，新增「时间轴跟弹」：预备一小节后按 BPM 连续推进，休止符按时经过；支持暂停续弹、区间与循环。
- 修复时间轴判定：以左侧固定播放轴前后各半拍作为识别带；原始事件时值与识别带相交期间可按严格乐谱顺序触发，完全离开后结算。单音触发目标音即命中，和弦曾同时包含全部目标音即命中，并允许额外音及区间内错后补对。
- 使用稳定事件 ID 保存待判定、当前已命中、最终正确和最终未通过状态；统计由已结算结果汇总为正确、未通过及通过率，未结算事件不进入分母。
- 实奏按键在五线谱以空心音符叠加显示；尚未命中时仅非目标音显示警告色，命中后不再警告。过去事件以主题正确／警告色配合勾叉保留结果，跨小节片段共享同一结果且只显示一个符号。
- Trainer 时间轴模式播放轴固定在可视区左侧 20%，谱面持续滚动，短谱及末尾也有足够留白；固定识别带取代随谱面移动的当前时值格边框。正式音符保持在时值格内居中，识别区间与音符中心解耦；窗口调整后恢复锚定，Editor 保持原有排版和安全区。
- 跟弹与示范共用 BPM，练习进度和示范定位独立。

验证：33 项自动化测试、构建与 Lint；覆盖本地采样锚点、识别带提前／延后输入、严格顺序、额外音、错后补对、和弦完整性、连续同音新触键、暂停与循环状态、区间裁剪、跨小节事件、谱面居中、反馈渲染及单一 RAF 调度。

### Trainer 乐谱跟练

- Editor 新增「去练习」，将当前内容（包括未保存编辑）复制到 Trainer；Trainer 也可载入本地已保存乐谱，不改写原谱。
- 新增乐谱跟练来源，保留随机单音 / 和弦练习及自由弹奏。跟练直接复用 Editor 的双谱表与安全区跟随、文档栏、主题和 Play Dock。
- 自由速度音高练习：和弦需完整同时匹配，全部松键后推进；错音或不完整和弦停留重试，休止符自动跳过，跨小节延音只计一个目标。
- 支持起止小节、整曲、循环、谱面选择起点、开始 / 暂停和重新开始；显示进度、首次正确率、错误次数、有效用时及上一轮成绩。
- 示范播放按所选小节范围裁剪，保留原时值与休止符，支持定位、进度拖动和 BPM 调整；与练习互斥且不计成绩。
- 切换工作区 / 练习来源或页面进入后台时暂停；练习状态与 Editor 数据保持隔离。
- 忽略未持有琴键的重复松键通知，避免鼠标移出琴键额外计错。
- 包版本和页脚更新为 0.6.0，乐谱文件格式仍为 version 1。

验证：构建、Lint 与 20 项自动化测试；浏览器验证乐谱交接、本地载入、错音重试、整轮成绩、分段循环与示范回放。USB / Bluetooth MIDI 硬件未实测。

Technical Debt：本阶段仅评价音高，未评价节奏；练习副本与成绩不持久化，尚无练习历史 / 错题库。沿用既有谱面排版、音频调度及实奏输入音频 ownership 技术债。详见 [TRAINER.md](TRAINER.md)。
...
- Dock 折叠与展开增加高度、透明度和页面预留空间的同步过渡动画，并遵循系统的减少动画偏好。
...
- Edit Dock 移除输入音／当前音下方的辅助小文本，将“第几项／第几拍”和末尾插入状态迁移到拍号旁的独立编辑位置列；两行音高内容保持单行，并同步压缩 Dock 纵向高度。
- 对齐编辑位置列与输入／当前两行的文字基线：编辑标签和项次落在第一行，拍位落在第二行，避免状态列内容垂直居中造成错位。
- 新建、保存、导出、打开文件统一固定规格；未保存时“保存”使用主题高亮色，保存后恢复，移除会改变按钮宽度的星号文本。
- File Dock 移除重复的“乐谱名称”标签，保留名称输入框并与摘要放在左侧同一行；新建、保存、导出、打开文件及历史按钮继续整体右对齐，文档栏纵向高度同步收紧。
- Web 页脚版本标识从旧开发版本更新为当前 `v0.5.0`。
...
- Play Dock 重组为进度、播放操作和 BPM 三个固定功能组：进度条与拍位保持同一行，播放与 BPM 控件统一为 34px 规格，并在两页共用相同的分隔和间距。

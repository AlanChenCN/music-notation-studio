...
- 保留逐音练习，新增「时间轴跟弹」：预备一小节后按 BPM 连续推进，错音 / 漏弹不停轴，休止符按时经过；支持暂停续弹、区间与循环。
- 跟弹在每个目标有效时段内检测完整音高，统计命中、漏弹与错音；连续同音需再次触键，当前不评价精确起音偏差和保持时长。
- 实奏按键在五线谱以强调色空心音符叠加显示，采用目标相同的纵向谱表几何；逐音模式靠近目标，时间轴模式靠近播放轴。
- Trainer 时间轴模式播放轴固定在可视区左侧 20%，谱面持续滚动，短谱及末尾也有足够留白；该模式音符按事件起点对齐播放轴。窗口调整后恢复锚定。Editor 保持原有排版和安全区。
- 跟弹与示范共用 BPM，练习进度和示范定位独立。

验证：27 项测试（含实奏谱面渲染），构建与 Lint；浏览器验证预备拍、无输入自动完成及漏弹、暂停示范定位、窄窗口 / 末尾左侧锚定。

...
Trainer 以 Editor 的布局与谱面为基础，新增 Editor 当前乐谱副本交接、本地保存乐谱载入、自由速度逐音练习与时间轴跟弹、实奏谱面音符、左侧固定跟弹轴、起止小节、循环及成绩反馈。时间轴以固定轴前后各半拍作为识别带：原始事件时值与识别带相交期间可按乐谱顺序触发，完全离开后结算；稳定事件 ID 保存当前命中、最终正确和最终未通过状态，谱面使用主题语义色与勾叉显示结果。正式音符保持时值格内居中，识别范围不依赖音符中心。Editor 与 Trainer 播放均使用单一帧同步时钟，避免重复定时器驱动 SVG。随机练习 / 自由弹奏继续独立保留。

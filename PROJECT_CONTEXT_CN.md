...
Trainer 以 Editor 的布局与谱面为基础，新增 Editor 当前乐谱副本交接、本地保存乐谱载入、自由速度逐音练习与时间轴跟弹、实奏谱面音符、左侧固定跟弹轴、起止小节、循环及成绩反馈。时间轴以固定轴前后各半拍作为识别带：原始事件时值与识别带相交期间可按乐谱顺序触发，完全离开后结算；稳定事件 ID 保存当前命中、最终正确和最终未通过状态，谱面使用主题语义色与勾叉显示结果。正式音符保持时值格内居中，识别范围不依赖音符中心。Editor 与 Trainer 播放均使用单一帧同步时钟，避免重复定时器驱动 SVG。随机练习现可按音域、音符池、和弦、拍号与时值生成标准乐谱，并复用同一跟练界面；自由弹奏独立提供即时谱面与最近试音。

`src/practice/scorePractice.ts` 管理逐音松键推进、时间轴固定识别带与严格顺序匹配、事件结果及统计，`ScoreTrainer.tsx` 管理跟练界面与区间，`PracticeTransport.tsx` 共用随机短句和乐谱示范回放。App 按工作区与练习来源分配实奏输入；示范声音仍经过 playback 路由，不计练习成绩。练习副本与会话结果不改写 Editor；文件模型仍为 version 1。

当前范围、操作与技术债详见 [TRAINER.md](TRAINER.md)。以下 v0.5.0 及更早章节保留发布历史。
...
回放控制位于乐谱下方，Play Dock 按进度、播放操作和 BPM 分组排列；进度条与拍位保持同一行，Trainer / Editor 共用统一按钮尺寸和分隔样式。支持进度条拖动、准星图标定位到当前事件及单按钮播放 / 暂停；BPM 可直接输入或每次 ±10，编辑光标与播放位置独立。
...
File Dock 保留可编辑乐谱名称和摘要在左侧，文件操作与撤销／重做整体右对齐并保持单行；名称标签文字已移除以节省高度。

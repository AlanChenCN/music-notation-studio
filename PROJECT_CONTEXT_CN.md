...
Trainer 以 Editor 的布局与谱面为基础，新增 Editor 当前乐谱副本交接、本地保存乐谱载入、自由速度逐音练习与时间轴跟弹、实奏谱面音符、左侧固定跟弹轴、起止小节、循环及成绩反馈。时间轴按原始事件的完整时值窗口识别，并以稳定事件 ID 保存当前命中、最终正确和最终未通过状态；谱面使用主题语义色与勾叉显示结果。正式音符保持时值格内居中，播放轴位置不改变识别窗口。随机练习 / 自由弹奏继续独立保留。

`src/practice/scorePractice.ts` 管理逐音松键推进、时间轴事件窗口匹配、事件结果与统计，`ScoreTrainer.tsx` 管理跟练界面与区间，`PracticeTransport.tsx` 共用随机短句和乐谱示范回放。App 按工作区与练习来源分配实奏输入；示范声音仍经过 playback 路由，不计练习成绩。练习副本与会话结果不改写 Editor；文件模型仍为 version 1。
